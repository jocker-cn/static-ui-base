...
export default function BulletproofLayout() {
    // 依然是同步读取，享受单例带来的纯净逻辑
    const config = ConfigurationManager.getConfig(SidebarConfig);
    const sidebarItems = config.items;
    const mockData = [
        { code: '001122', name: 'Tech Innovation Fund', type: 'equity' },
        { code: '003344', name: 'Global Bond A', type: 'bond' }
    ] as const;
    return (
        <div style={{display: 'flex', height: '100vh', width: '100vw', fontFamily: 'sans-serif'}}>
            ...
            <aside style={{
              ...
              {/*  <div style={{padding: '24px', fontSize: '20px', fontWeight: 'bold', borderBottom: '1px solid #34495e'}}>
                ...
                {/*                    {item.children.map(child => <div key={child.key}*/}
                {/*                                                     style={{padding: '4px 0'}}>{child.title}</div>)}*/}
                {/*                </div>*/}
                {/*            )}*/}
                {/*        </div>*/}
                {/*    ))}*/}
